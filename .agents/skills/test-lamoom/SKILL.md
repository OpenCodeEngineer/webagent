...
description: >
  ...
  "does it work", "qa check", "test ui", "check the ui", "test lamoom",
  "lamoom e2e", "verify lamoom", "check lamoom".
...
## Auth Gotchas (learned 2026-05-23)

When running programmatic tests (Playwright scripts, not real browser login), these are the failure modes discovered:

### JWT must include `id` field (not just `sub`)

NextAuth v5 `jwt` callback stores user ID as `token.id`, not `token.sub`:
```typescript
// packages/admin/src/lib/auth.ts
async jwt({ token, user }) {
  if (user?.id) token.id = user.id;  // stored as token.id
  ...
}
async session({ session, token }) {
  if (token.id) session.user.id = token.id;  // reads token.id, NOT token.sub
  ...
}
```
Without `id` in the JWT payload: `session.user.id` is undefined → `/api/auth/ws-ticket` returns 401 → WS auth fails → textarea on `/create` stays disabled forever.

Generate JWT with both fields:
```javascript
token: { sub: USER_ID, id: USER_ID, ... }
...
```
Not on `browser.contexts()[0]` — that throws or silently fails.

### Cookie name depends on `AUTH_URL`

`AUTH_URL=https://...` → cookie name `__Secure-authjs.session-token` (requires `secure: true`)
`AUTH_URL=http://...` → cookie name `authjs.session-token`

Check `/opt/webagent/.env` on server before setting cookie name in scripts.

### Stale embed tokens on `/create`

The `/create` page renders full chat history including embed codes from previous conversations. A naive `page.$('[data-agent-token]')` returns old tokens. Always:
1. Snapshot all existing tokens BEFORE sending the test message
2. Wait for a token NOT in that set after sending
...
### Textarea disabled until WS auth succeeds

On `/create`, textarea starts `disabled`. It only enables after the proxy WS auth succeeds (ws-ticket flow). Wait for:
```javascript
await page.waitForSelector('textarea:not([disabled])', { timeout: 15000 });
```
If it never enables: check `/api/auth/session` has `user.id`, then check `/api/auth/ws-ticket` returns 200.
